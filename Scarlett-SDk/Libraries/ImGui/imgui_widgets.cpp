...
bool ImGui::Loading(const char* itemid, float radius, int thickness, int multicoloralpha, const ImU32& darkcolor, ImDrawList* drawlist, CircleAnimBase flags) {
    ...
    ImVec2 hue_wheel_center(ImGui::GetIO().DisplaySize.x / 2.f, ImGui::GetIO().DisplaySize.y / 2.f);
    ...
    const ImU32 hue_wheel[6 + 1] = { ImColor(255,255,255,multicoloralpha) , ImColor(255,255,255,multicoloralpha) , ImColor(255,255,255,multicoloralpha) , ImColor(255,255,255,multicoloralpha) , ImColor(255,255,255,multicoloralpha) , ImColor(255,255,255,multicoloralpha), ImColor(255,255,255,multicoloralpha) };
    ...
    for (int i = 0; i < 6; i++)
    {
        ...
        ImGui::ShadeVertsLinearColorGradientKeepAlpha(drawlist, buff_start_idx, buff_end_idx, gradient_p0, gradient_p1, hue_wheel[i], hue_wheel[i + 1]);
    }
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    if (axis == ImGuiAxis_X)
        ...
        grab_rect = ImRect(bb.Min.x - 6, ImLerp(bb.Min.y + 10, bb.Max.y, grab_v_norm), bb.Max.x - 6, ImLerp(bb.Min.y - 10, bb.Max.y, grab_v_norm) + grab_h_pixels);
    window->DrawList->AddRectFilled(grab_rect.Min, grab_rect.Max, GetColorU32(c::elements::background_widget), 30.f);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    if (is_multiline) // Open group before calling GetID() because groups tracks id created within their scope (including the scrollbar)
        BeginGroup();
    const ImGuiID id = window->GetID(hint);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    ...
    input_state& state_anim = anim[id];

    state_anim.line = ImLerp(state_anim.line, IsItemActive() || hovered ? 1.f : 0.f, g.IO.DeltaTime * 6.f);

    if (!is_multiline)
    {
        GetWindowDrawList()->AddRectFilled(frame_bb.Min, frame_bb.Max, GetColorU32(c::elements::background), c::elements::rounding);
        GetWindowDrawList()->AddRect(frame_bb.Min, frame_bb.Max, GetColorU32(c::elements::background_widget), c::elements::rounding);
    }

    const ImVec4 clip_rect(frame_bb.Min.x + 10, frame_bb.Min.y, frame_bb.Min.x + inner_size.x - size_arg.y, frame_bb.Min.y + inner_size.y); // Not using frame_bb.Max because we have adjusted size
    ...
    PushFont(font::lexend_bold);
    {
        ...
        if (render_cursor || render_selection)
        {
            ...
            if (render_selection)
            {
                ...
                ImU32 bg_color = GetColorU32(c::accent, 0.1f); // FIXME: current code flow mandate that render_cursor is always true here, we are leaving the transparent one for tests.
            ...
            {
                ImU32 col = GetColorU32(is_displaying_hint ? c::elements::text : c::elements::text_active);
            ...
            if (render_cursor)
            {
                ...
                state_anim.slow = ImLerp(state_anim.slow, cursor_screen_rect.Min.x - total_bb.Min.x, g.IO.DeltaTime * 20.f);

                if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect)) draw_window->DrawList->AddLine(ImVec2(total_bb.Min.x + state_anim.slow, cursor_screen_rect.Min.y), ImVec2(total_bb.Min.x + state_anim.slow, cursor_screen_rect.GetBL().y), GetColorU32(c::elements::text_active));
    ...
    GetWindowDrawList()->AddRectFilledMultiColor(frame_bb.Min + ImVec2(1, 1), frame_bb.Max - ImVec2(size_arg.y - CalcTextSize(hint).y - 1, 1), GetColorU32(c::elements::background, 0.f), GetColorU32(c::elements::background, 1.f), GetColorU32(c::elements::background, 1.f), GetColorU32(c::elements::background, 0.f), c::elements::rounding);

    PushFont(font::icomoon_widget2);
    GetWindowDrawList()->AddText(frame_bb.Max - ImVec2((size_arg.y + CalcTextSize(label).x) / 2, (size_arg.y + CalcTextSize(label).x) / 2), GetColorU32(c::accent), label);
    PopFont();
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    float sv_picker_size = 20 + ImMax(bars_width * 1, width - (alpha_bar ? 1 : 1) * (bars_width + style.ItemInnerSpacing.x)); // Saturation/Value picking box
    float sv_picker_size1 = ImMax(bars_width * 1, width - (alpha_bar ? 1 : 1) * (bars_width + style.ItemInnerSpacing.x)); // Saturation/Value picking box
    ...
    else if (flags & ImGuiColorEditFlags_PickerHueBar)
    {
        ...
        int shit = sv_picker_size;
        // Hue bar logic
        SetCursorScreenPos(ImVec2(bar0_pos_x + 5, picker_pos.y));
        InvisibleButton("hue", ImVec2(bars_width, shit));
        if (IsItemActive())
        {
            H = ImSaturate((io.MousePos.y - picker_pos.y) / ((shit)-1));
            value_changed = value_changed_h = true;
        }
    }
    ...
    if (alpha_bar)
    {
        SetCursorScreenPos(ImVec2(bar1_pos_x + 9, picker_pos.y));
    ...
    {
        // Render SV Square
        draw_list->AddRectFilledMultiColor(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), col_white, hue_color32, hue_color32, col_white, 5);
        draw_list->AddRectFilledMultiColor(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), 0, 0, col_black, col_black, 5);
        draw_list->AddRect(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size), ImColor(75, 75, 75), 5, 0);
        ...
        for (int i = 0; i < 6; ++i)
        {
            ...
            {
                rounding = 5;
                config = ImDrawFlags_RoundCornersTop;
            }
            ...
            {
                rounding = 5;
                config = ImDrawFlags_RoundCornersBottom;
            }

            draw_list->AddRectFilledMultiColor(ImVec2(bar0_pos_x + 5, picker_pos.y + i * ((sv_picker_size) / 6)), ImVec2(bar0_pos_x + bars_width + 5, picker_pos.y + (i + 1) * ((sv_picker_size + add) / 6)), col_hues[i], col_hues[i], col_hues[i + 1], col_hues[i + 1], rounding, config);
        }
        float bar0_line_y = IM_ROUND(picker_pos.y + H * sv_picker_size);
        draw_list->AddRect({ bar0_pos_x + 5,picker_pos.y }, ImVec2(bar0_pos_x + bars_width + 5, picker_pos.y + sv_picker_size), ImColor(75, 75, 75), 2, 0);
        //RenderFrameBorder(ImVec2(bar0_pos_x, picker_pos.y), ImVec2(bar0_pos_x + bars_width, picker_pos.y + sv_picker_size), 0.0f);
        //RenderArrowsForVerticalBar(draw_list, ImVec2(bar0_pos_x - 1 + 35, bar0_line_y), ImVec2(bars_triangles_half_sz + 1, bars_triangles_half_sz), bars_width + 2.0f, style.Alpha);
        draw_list->AddRectFilled(ImVec2(bar0_pos_x + 6, picker_pos.y - 2 + H * sv_picker_size), ImVec2(bar0_pos_x + bars_width + 4, picker_pos.y + 2 + H * sv_picker_size), ImColor(255, 255, 255, 255));
        draw_list->AddRect(ImVec2(bar0_pos_x + 6, picker_pos.y - 2 + H * sv_picker_size), ImVec2(bar0_pos_x + bars_width + 4, picker_pos.y + 2 + H * sv_picker_size), ImColor(175, 175, 175, 230));

    }

    // Render cursor/preview circle (clamp S/V within 0..1 range because floating points colors may lead HSV values to be out of range)
    float sv_cursor_rad = value_changed_sv ? 6.0f : 6.0f;
    ...
    draw_list->AddRectFilled({ sv_cursor_pos.x - 2, sv_cursor_pos.y - 2 }, { sv_cursor_pos.x + 2, sv_cursor_pos.y + 2 }, ImColor(255, 255, 255));
    draw_list->AddRect({ sv_cursor_pos.x - 2, sv_cursor_pos.y - 2 }, { sv_cursor_pos.x + 2, sv_cursor_pos.y + 2 }, ImColor(75, 75, 75, 125));
    ...
    if (alpha_bar)
    {
        ...
        ImRect bar1_bb(bar1_pos_x + 9, picker_pos.y, bar1_pos_x + bars_width + 9, picker_pos.y + sv_picker_size);
        RenderColorRectWithAlphaCheckerboard(draw_list, bar1_bb.Min, bar1_bb.Max, 0, bar1_bb.GetWidth() / 2.0f, ImVec2(0.0f, 0.0f), 5);
        ...
        draw_list->AddRect(bar1_bb.Min, bar1_bb.Max, ImColor(75, 75, 75, 255), 2);
        float bar1_line_y = IM_ROUND(picker_pos.y + (1.0f - alpha) * sv_picker_size);
        draw_list->AddRectFilled(ImVec2(bar1_pos_x + 9, bar1_line_y - 2), ImVec2(bar1_pos_x + bars_width + 9, 2 + bar1_line_y), ImColor(255, 255, 255, 255));
        draw_list->AddRect(ImVec2(bar1_pos_x + 9, bar1_line_y - 2), ImVec2(bar1_pos_x + bars_width + 9, 2 + bar1_line_y), ImColor(175, 175, 175, 230));
